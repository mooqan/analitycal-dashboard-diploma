...
const Dashboard: FC = () => {
	...
	const handleNewSalesChange = (e: React.ChangeEvent<HTMLInputElement>) =>
		setNewSales(Number(e.target.value))
	const handleNewRevenueChange = (e: React.ChangeEvent<HTMLInputElement>) =>
		setNewRevenue(Number(e.target.value))
	const handleNewOrdersChange = (e: React.ChangeEvent<HTMLInputElement>) =>
		setNewOrders(Number(e.target.value))
	...
	const handleAnalyzeData = async () => {
		try {
			...
			navigate('/analysis-report', {
				state: { recommendations: recommendationsResponse },
			})
		} catch (error) {
	...
	return (
		<div className="container mx-auto p-4">
			<div className="flex justify-between items-center mb-4">
				<button onClick={handleAnalyzeData} className="btn btn-blue py-2 px-4 rounded-md bg-blue-600 hover:bg-blue-700 text-white">
    Анализ данных
</button>
			...
			<div className="date-filters flex justify-center gap-4 mb-5">
				<div className="flex flex-col items-center">
					<label className="mb-2 text-white">Начало периода</label>
					<DatePicker
						selected={startDate}
				...
				<div className="flex flex-col items-center">
					<label className="mb-2 text-white">Конец периода</label>
					<DatePicker
						selected={endDate}
				...
				<button onClick={handleApplyDates} className="btn btn-green">
					Применить даты
				</button>
			</div>
			...
			>
				<div className="flex flex-col items-center">
					<label className="mb-2 text-white">Выберите дату</label>
					<DatePicker
						selected={newDate}
				...
				<div className="flex flex-col items-center">
					<label className="mb-2 text-white">Продажи</label>
					<input
						type="number"
						value={newSales}
						onChange={handleNewSalesChange}
						className="input"
					/>
				</div>
				<div className="flex flex-col items-center">
					<label className="mb-2 text-white">Доходы</label>
					<input
						type="number"
						value={newRevenue}
						onChange={handleNewRevenueChange}
						className="input"
					/>
				</div>
				<div className="flex flex-col items-center">
					<label className="mb-2 text-white">Заказы</label>
					<input
						type="number"
						value={newOrders}
